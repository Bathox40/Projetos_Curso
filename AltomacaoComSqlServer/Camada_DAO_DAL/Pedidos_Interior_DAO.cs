...
{
    public class Pedidos_Interior_DAO : DAO_DAO
    {
        ...
        public override DataTable ConsultarBD(Object objvo_VO)
        {
            try
            {
                Pedidos_Interior_VO objParPedidos_Interior_VO = (Pedidos_Interior_VO)objvo_VO;
                ...
                if (!objParPedidos_Interior_VO.ID.Equals(0))
                {
                    strSql.Append(" WHERE");
                    strSql.Append(" ID = ?");
                    ...
                    objComando.Parameters["?ID"].Value = objParPedidos_Interior_VO.ID;
                }
                else if (!string.IsNullOrEmpty(objParPedidos_Interior_VO.Descricao))
                {
                    strSql.Append(" WHERE");
                    strSql.Append(" Descricao = ?");
                    ...
                    objComando.Parameters["?Descricao"].Value = objParPedidos_Interior_VO.Descricao;
                }
                else if (!objParPedidos_Interior_VO.Cliente_ID.ID.Equals(0))
                {
                    strSql.Append(" WHERE");
                    strSql.Append(" Cliente_ID = ?");
                    ...
                    objComando.Parameters["?Cliente_ID"].Value = objParPedidos_Interior_VO.Cliente_ID.ID;
                }
                else
                {
                    objComando = new OleDbCommand(strSql.ToString(), getConexao());
        ...
        public override bool IncluirBD(Object objvo_VO)
        {
            try
            {
                ...
                objComando.Parameters["?Estado"].Value = objParPedidos_Interior_VO.ID;
        ...
        public override bool AlterarBD(Object objvo_VO)
        {
            try
            {
                ...
                objComando.Parameters.Add("?Estado", OleDbType.SmallInt);
                objComando.Parameters["?Estado"].Value = objParPedidos_Interior_VO.Estado;
